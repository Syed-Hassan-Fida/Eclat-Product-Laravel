...
class ProductOrderController extends Controller
{
    ...
    private function common_charge_customer_data($order_details,$ipn_route,$payment_type = 'product') : array
    {
        $data = [
          ...
          //   'ipn_url' => route(self::SUCCESS_ROUTE, random_int(333333, 999999) . $order_details->id . random_int(333333, 999999)),
       ...
       // echo 'adsasd';
         //dd($order_details->transaction_id);
        
       // Session::set('$order_details', $data['success_url']);
        session()->put('success_url_stripe',$data['success_url']);
        session()->put('order_id_stripe', $order_details->id);
        session()->put('transaction_id', $order_details->transaction_id);
    ...
    {
         //  dd(session()->get('transaction_id_stripe'));
       // dd(session()->get('order_id_stripe'));
        
       if(session()->get('order_id_stripe')){
               event(new Events\ProductOrders([
                'order_id' => session()->get('order_id_stripe'),
                'transaction_id' => Str::random(6)
            ]));

            $order_id = Str::random(6) . session()->get('order_id_stripe'). Str::random(6);
            return redirect()->route(self::SUCCESS_ROUTE, $order_id);
            
        }else if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
